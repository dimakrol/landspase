                ...
                <div class="col-lg-8">
                  ...
                  @elseif($setting->type == 'checkbox')
                    ...
                    @foreach($setting->getSettingDropDownOptions() as $key => $value)
                      ...
                      <div class="i-checks"><label>
                        {{ Form::checkbox('settings['.$setting->id.']['.$key.']', $value, $setting->isChecked($key)) }} <i></i> {{$value}}
                      </label></div>
                    @endforeach
                  @elseif($setting->type == 'radio')
                    ...
                    @foreach($setting->getSettingDropDownOptions() as $key => $value)
                      {{ Form::hidden('settings['.$setting->id.']', 0) }}                                                                                                                             
                      <div class="i-checks"><label>
                        {{ Form::radio('settings['.$setting->id.']', $value, $key == $setting->getValue()) }} <i></i> {{$value}}
                      </label></div>
                    @endforeach
                  ...
                  @elseif($setting->type == 'yesno')
                    <div class="i-checks"><label>
                      {{ Form::radio('settings['.$setting->id.']', 1, $setting->getValue()) }} <i></i> Yes
                    </label></div>
                    <div class="i-checks"><label>
                      {{ Form::radio('settings['.$setting->id.']', 0, !$setting->getValue()) }} <i></i> No
                    </label></div>
